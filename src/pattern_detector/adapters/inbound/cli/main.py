"""Typer CLI interface for DPX-OCaml."""
...
def scan(
    path: Annotated[
        str,
        typer.Argument(
            help="Path to an OCaml source/interface file (.ml / .mli) or directory containing a Dune project/library.",
        ),
    ] = ".",
    ...
    pattern: Annotated[
        ...
        typer.Option(
            "--pattern",
            "-p",
            help="Filter for specific pattern type(s) (e.g. 'functor_parametric_module', 'gadts_expression_eval', 'effect_handlers', 'railway_monadic_bind').",
        ),
    ] = None,
    ...
    """Scan an OCaml codebase for Functors, GADTs, Effect Handlers, and Type Safety vulnerabilities."""
...
def _handle_terminal_scan(
    ...
    # 1. Header Banner
    header = Panel.fit(
        f"[bold white]🐫 DPX-OCaml: Module Architecture & Functional Pattern Detector • Hexagonal DDD[/bold white]\n"
    ...
    table = Table(title="📊 Detection Summary by Category", header_style="bold orange3")
    ...
    console.print("[bold]📋 Identified Design Patterns & Functional Idioms:[/bold]\n")
    for idx, d in enumerate(report.detections, 1):
        ...
        header_text = f"[bold orange3]#{idx} {d.pattern_type.value.upper()}[/bold orange3] on [bold]{d.target_kind}[/bold] '[yellow]{d.target_name}[/yellow]'"
        ...
        for i, ev in enumerate(d.evidences):
            ...
            lines.append(f"{connector} [orange3]+{int(ev.weight * 100)}%[/orange3] ([dim]{ev.rule_code}[/dim]) {ev.description}{ev_loc}")
...
@app.command()
def rules() -> None:
    """List all supported OCaml design patterns, module idioms, and safety rules."""
    table = Table(title="🐫 Supported Patterns & Rules in DPX-OCaml", header_style="bold orange3")
...
@app.command()
def info() -> None:
    """Display DPX-OCaml architecture, version, and grammar engine details."""
    panel = Panel(
        """[bold orange3]🐫 DPX-OCaml • Module Architecture & Functional Pattern Detector[/bold orange3]
[dim]Multi-Paradigm Hexagonal Pattern Detector & OCaml AST/CST Engine[/dim]

• [bold]Target Language:[/bold] OCaml (OCaml 4.14 - 5.0 - 5.3+ / Multicore)
• [bold]Architecture:[/bold] Hexagonal Ports & Adapters + Domain-Driven Design (DDD)
• [bold]Rules Supported:[/bold] 25 Rules (Functors, GADTs, Effect Handlers, Monadic Bind, ADT Interfaces)
